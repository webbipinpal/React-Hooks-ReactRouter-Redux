...
const AvailableMeals = () => {
  ...
  useEffect( () => {
    ...
    const fatchMeals = async () => {
        const response = await fetch('https://myudemy-react-default-rtdb.asia-southeast1.firebasedatabase.app/meals.json');

        if(!response.ok){
          throw new Error('Somthing is missing from Servar');
        }
